...
namespace OS_project2
{
    class File_Entry : Directory_Entry
    {
        ...
        public File_Entry(string name, byte dir_attr, int dir_firstCluster, int fileSize, string Content, Directory pa)
            : base(name, dir_attr, dir_firstCluster, fileSize)
        {
            content = Content;
            if (pa != null)
                parent = pa;
        ...
        public void writeFileContent()
        {
            ...
            {
                clusterFATIndex = this.File_FirstCluster;
            }
            else
            {
                clusterFATIndex = Fat_Table.Getavaliableblock();
                this.File_FirstCluster = clusterFATIndex;
            }
            int lastCluster = -1;
        ...
        public void readFileContent()
        {
            if (this.File_FirstCluster != 0)
            {
                int cluster = this.File_FirstCluster;
        ...
        public void deleteFile()
        {
            ...
            if (this.parent != null)
            {
                int index = this.parent.searchDirectory(new string(this.File_Or_DirName));
                if (index != -1)
                {
